...
class Processor:
    ...
    def full_process(self, image_name_list):
        self.build(image_name_list)
        self.fusion(image_name_list)
        self.analysis(image_name_list)
    ...
    def build(self, image_name_list):
        ...
        for image_filename in image_name_list:
            ...
            if not images.check_pickle(image_name):
                image = images.get_original(image_name) #todo: qui si genera l'errore
                try:
    ...
    def fusion(self, image_name_list):
        topic = self.conf.get('ntfy_topic')
        ...
        for image_filename in image_name_list:
            ...
            images.clean_fusion(image_filename, fusion_channel)
    ...
    def analysis(self, image_name_list):
        topic = self.conf.get('ntfy_topic')
        images = State(self.conf)
        images.load_pickle()
